...
df_owner = pd.DataFrame(data_owner, columns=[
                        'owner_name', 'address', 'city', 'state', 'zipcode'])
df_owner.to_sql(con=engine, name='owner', if_exists='append',
                index=False)  # Releasing BATCH to MySQL
...
df_puppies = pd.DataFrame(data_puppies, columns=[
